...
export const More = () => {
    ...
    return (
        isLoading ? <Loading /> :
            <search className='h-[80vh] w-full backdrop-blur-lg flex justify-center items-center scroll-auto'>
                <section className='h-1/2 w-3/4 rounded-2xl flex flex-col overflow-y-scroll border border-amber-400 p-2'>
                    {MoreIterator(router, logout, showLoading, hideLoading, clearCart, setView, user?._id ?? undefined).map((item, index) => (
